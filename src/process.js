...
export function runProcess(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { cwd: options.cwd, env: { ...process.env, ...(options.env || {}) }, shell: shouldUseShell(command, options), windowsHide: true, stdio: ['pipe', 'pipe', 'pipe'] });
...
export function shouldUseShell(command, options = {}, platform = process.platform) {
