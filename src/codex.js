...
export async function runCodexPhase({ kind, prompt, root, runDir, model, reasoning, sandbox, timeoutMs, codexCommand, approveForMe = false, attempt = 1, runner = runProcess, commandResolver = resolveCodexCommand, repair = false }) {
  ...
  args.push('-');
  const executable = commandResolver({ configuredCommand: codexCommand });
  let events = ''; await runner(executable, args, { cwd: root, timeoutMs, input: `[CODEX_FEATURE_PIPELINE_PHASE]\n${prompt}`, env: { CODEX_FEATURE_PIPELINE_ACTIVE: '1' }, onStdout: (text) => { events += text; } }); fs.writeFileSync(eventsPath, events);
  try {
